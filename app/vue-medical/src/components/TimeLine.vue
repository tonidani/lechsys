   ...
   data: () => ({
    ...
    methods: { 
       ...
       //pobieranie api i wstepna obrobka 
        ...
        let res2 = await fetch('/api/users/'+player_id+'/kinase')
        ...
        let DDDD = YYYY + "-" + MM + "-" + DD
         ...
         if (data.length == 0 && data2.length == 0){
          ...
          let dataa = {color : 'purple',description: 'No Data',details : 'No Data',end : DDDD,has_kinas: true,id : 1,latitude : null,longitude : null,name : 'Brak danych',start : DDDD,type_id : 4 }
          data.push(dataa)
          let dataa2 = {date: DDDD, event_id: 1, id: 1, state: 'COMPLETED', value: 0 }
          data2.push(dataa2)
        }
      ...
      for (let ye=0; ye<=calendat_year1; ye++){ 
        for(let ddd=0; ddd<data.length; ddd++){ 
          if (data[ddd].end.slice(0,4) == 2020 + ye && data[ddd].has_kinase == true)
      ...
      for (let yee=0; yee<=calendat_year1; yee++){ 
       ...
       let months = [], month1=[], month2=[], month3=[] ,month4=[], month5=[], month6=[], month7=[], month8=[] ,month9=[], month10=[], month11= [], month12 =[]
        for (let d=0; d<table_year[yee].length; d++){ 
          if (table_year[yee][d].has_kinase == true ){
        ...
        for (let al = 0; al<months.length; al++){ 
          for (let al2 = 0; al2<months[al].length; al2++){ 
            if (months[al][al2].has_kinase == true){ 
