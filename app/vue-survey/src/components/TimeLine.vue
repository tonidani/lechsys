...
<v-app> 
  ...
  >
    ...
    <div class="text-center">
      ...
      class="ms-10">
          <v-col cols="auto">
              <v-btn @click="minus_month(); get_data_table(motoric[month_id],loading)"
                class="mx-2"
                fab
                dark
                small
           ...
           <v-col cols="auto">
              <v-btn @click="plus_month(); get_data_table(motoric[month_id],loading)"
                class="mx-2"
                fab
                dark
                small
   ...
   data: () => ({
    ...
    methods: { 
       async get_events(){ 
        ...
        let res2 = await fetch('/api/users/'+player_id+'/wellness')
        let data2 = await res2.json()
 
        let res3 = await fetch('/api/users/'+player_id+'/rpe')
        let data3 = await res3.json()

        let res4 = await fetch('/language')
        let data4 = await res4.text()
        this.$i18n.locale = data4
        this.lang = data4
        ...
        let DDDD = YYYY + "-" + MM + "-" + DD
         ...
         if (data.length == 0 && data2.length == 0 && data3.length ==0 ){
          ...
          let dataa = {color : 'purple',description: 'No Data',details : 'No Data',end : DDDD ,has_kinas: false,id : 1,latitude : null,longitude : null,name : 'Brak danych',start : DDDD ,type_id : 5 }
          data.push(dataa)
          let dataa2 = {date : DDDD ,energy : 0,event_id : 4,fatigue : 0,id : 1,mood : 0,muscle_soreness : 0,rpe : 0,sleep_quality : 0,state : 'TO-COMPLETE'}
          data2.push(dataa2)
          let dataa3 = {date : DDDD ,event_id : 1,id : 1,state :  'TO-COMPLETE',total_time :'00:00:00',value : 0  }
          data3.push(dataa3)
        }
      ...
      for (let ye=0; ye<=calendat_year1; ye++){ 
        for(let ddd=0; ddd<data.length; ddd++){ 
          if (data[ddd].end.slice(0,4) == 2020 + ye)
          ...
          for (let al2 = 0; al2<months[al].length; al2++){ 
              ...
              for (let moto = 0; moto<motoryczne_dane.length; moto++){ 
                if (months[al][al2].end == motoryczne_dane[moto].date)
                  glowna_miesiac.push(motoryczne_dane[moto])
              }
      ...
      this.get_data_table(this.motoric[this.month_id],this.loading)
        }
      }, 
      minus_month(){ 
